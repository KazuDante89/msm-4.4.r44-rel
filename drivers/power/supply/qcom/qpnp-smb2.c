...
static enum power_supply_property smb2_batt_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGING_ENABLED,//lct add for runin 20180815
	#endif
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TECHNOLOGY:
		val->intval = POWER_SUPPLY_TECHNOLOGY_LIPO;//xiaomi need
		break;
...
static int smb2_init_hw(struct smb2 *chip)
{
	...
	/*set aicl start at 4.5V,only d2s*/
	#if defined CONFIG_KERNEL_CUSTOM_D2S
	rc = smblib_masked_write(chg, USBIN_5V_AICL_THRESHOLD_CFG_REG,
                                USBIN_5V_AICL_THRESHOLD_CFG_MASK, 0x5);
        if (rc < 0) {
                pr_err("Couldn't set 5V aicl threshold  rc=%d\n", rc);
                return rc;
        }
	#endif
...
static void thermal_fb_notifier_resume_work(struct work_struct *work)
{
...
#elif defined(CONFIG_KERNEL_CUSTOM_E7T)
	if (LctIsInCall == 1)
		smblib_set_prop_system_temp_level(chg,&lct_therm_call_level);//level 5, 1000mA
	else //lcd on
		smblib_set_prop_system_temp_level(chg,&lct_therm_lvl_reserved);//from thermal-level
	LctThermal = 0;
	...
	{
		if (lct_therm_lvl_reserved.intval >= 2)
			smblib_set_prop_system_temp_level(chg,&lct_therm_globe_level);//level 2
		else
			smblib_set_prop_system_temp_level(chg,&lct_therm_level);//level 0
	}
	else if (LctIsInCall == 1)
		smblib_set_prop_system_temp_level(chg,&lct_therm_call_level);//level 3
	else
	...
	{
		if (lct_therm_lvl_reserved.intval >= 1)
			smblib_set_prop_system_temp_level(chg,&lct_therm_india_level);//level 1
		else
			smblib_set_prop_system_temp_level(chg,&lct_therm_level);//level 0
	}
	else if (LctIsInCall == 1)
