...
#define JEITA_CC_COMP_CFG_IN_UEFI  1200  //if changed JeitaCcCompCfg in uefi,we need change this value same as uefi
...
static int smblib_adjust_jeita_cc_config(struct smb_charger *chg,int val_u)
{
	...
	if((val_u == chg->batt_profile_fcc_ua) && (current_cc_minus_ua != JEITA_CC_COMP_CFG_IN_UEFI * 1000 ) ) //fcc is enqual  2.5A,if jeita cc config is not equal UEFI config,we need set it back
	{
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	smblib_err(chg, "lct IRQ: usbin-plugin %s\n",
					vbus_rising ? "attached" : "detached");//lct log
#ifdef CONFIG_KERNEL_CUSTOM_F7A
	if (g_nvt.valid) {
...
static void smblib_force_legacy_icl(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB_FLOAT:
		 ...
		 */
		#if defined (CONFIG_KERNEL_CUSTOM_E7S)
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, 500000);//xiaomi nedd
